...
public class MainActivity extends AppCompatActivity {
    ...
    public void gettingWeather(View view){
        ...
        if(search_bar.getText().toString().trim().equals("") || search_bar.getText().toString().trim().equals(null)){
            Toast.makeText(this, "Please Enter Something To Search", Toast.LENGTH_SHORT).show();
        }else {

//            queue.push(search_bar.getText().toString().trim());
//
...
//            if(queue.size()>=2){
//                queue.removeLast();
//                if (!queue.peekLast().equals(search_bar.getText().toString().trim())) changeBackgroundImage();
//
//                queue.push(search_bar.getText().toString().trim());
//            }else
            ...
            exampleCall.enqueue(new Callback<Example>() {
                ...
                public void onResponse(Call<Example> call, retrofit2.Response<Example> response) {
                    if (response.code() == 404) {
                        ...
                        else if (windDirection >= 33.75 && windDirection < 78.75)
                            dir.append("North East");
                        else if (windDirection >= 78.75 && windDirection < 123.75)
                            dir.append("East");
                        else if (windDirection >= 123.75 && windDirection < 168.75)
                            dir.append("South East");
                        else if (windDirection >= 168.75 && windDirection < 213.75)
                            dir.append("South");
                        else if (windDirection >= 213.75 && windDirection < 258.75)
                            dir.append("South West");
                        else if (windDirection >= 258.75 && windDirection < 303.75)
                            dir.append("West");
                        else if (windDirection >= 303.75 && windDirection < 348.75)
                            dir.append("North West");
...
//                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());
                        ...
                        if (stringBuilder.substring(0, 3).equals("Mon"))
                            currentDay.append("Monday");
                        else if (stringBuilder.substring(0, 3).equals("Tue"))
                            currentDay.append("Tuesday");
                        else if (stringBuilder.substring(0, 3).equals("Wed"))
                            currentDay.append("Wednesday");
                        else if (stringBuilder.substring(0, 3).equals("Thu"))
                            currentDay.append("Thursday");
                        else if (stringBuilder.substring(0, 3).equals("Fri"))
                            currentDay.append("Friday");
                        else if (stringBuilder.substring(0, 3).equals("Sat"))
                            currentDay.append("Saturday");
                        else if (stringBuilder.substring(0, 3).equals("Sun"))
                            currentDay.append("Sunday");
